...
for m = 1%:N_movie
    ...
    for j = 1:size(merged_itraces{m,ch},1) %cycle through spots, in iEndval ascending order
        ...
        y_0_end = round(y_0 + ch1{m}.drift(end,2));
...
for m=1:N_movie
    for ch = 1:2
    for i = 1:size(vwcm_result{m},1)
        vwcm_result{m}{i,ch}(:,6:13) = output{m}{i,ch};
    end
    end
end     
        
%% Mapping - map estimated coordinates on other channel
if mapping
    for m = 1:N_movie
    for i = 1:size(vwcm_result{m},1)
    for j = 1:size(vwcm_result{m}{i,1},1)
        if vwcm_result{m}{i,1}(j,1)~=0;
